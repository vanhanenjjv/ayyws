...
describe('handler', () => {
  it('should return batch item failure for retried record', async () => {
    const handler = sqs.handler(vi.fn().mockResolvedValueOnce(sqs.retry()))
  ...
  it('should not return batch item failure for passed record', async () => {
    const handler = sqs.handler(vi.fn().mockResolvedValueOnce(sqs.pass()))
    const event: SQSEvent = { Records: [{ messageId: '1' } as SQSRecord] }
    const response = await handler(event, {} as Context, () => { })
    expect(response).toEqual({
      batchItemFailures: []
  ...
  it('should reject when handler throws', async () => {
    const handler = sqs.handler(vi.fn().mockRejectedValueOnce(new Error('Unmanaged Error')))
    const event: SQSEvent = { Records: [{ messageId: '1' } as SQSRecord] }
